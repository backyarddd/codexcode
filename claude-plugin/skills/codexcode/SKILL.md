...
First, prepare Claude Code's review workspace:
...
Now, in this current Claude Code session, perform the host review yourself.
Read `prompt_file` and `diff_file`, follow `review_prompt`, and write the
review to `review_file`. Do not run `claude`, `claude -p`, or any other
separate Claude Code process for this host review. Modify only `review_file`
...
This runs Codex reviewing Claude Code's diff and writes the review to
`~/.codexcode/sessions/<session_id>/reviews/by_challenger.md`. It blocks until
the challenger review finishes. Claude Code's review was already produced in
this live session and saved to `by_host.md`.
